...
export default class Rect extends Tool {
  ...
  handleDrag(e, posn, lastPosn) {
      ...
      let frame = new HistoryFrame(
        [
          new actions.InsertAction({
            items: [{ item: this.rect, index: this.currentIndex }]
          }),
    ...
    if (
      (posn.x < this.scaleOrigin.x &&
        this.rect.bounds().l === this.scaleOrigin.x) ||
      (posn.x > this.scaleOrigin.x &&
        this.rect.bounds().r === this.scaleOrigin.x)
    ) {
      // Flip
      x *= -1;
    }
    ...
    if (
      (posn.y < this.scaleOrigin.y &&
        this.rect.bounds().t === this.scaleOrigin.y) ||
      (posn.y > this.scaleOrigin.y &&
        this.rect.bounds().b === this.scaleOrigin.y)
    ) {
      // Flip
      y *= -1;
    }
