...
export default class Polynomial {
  ...
  interpolate(xs, xy, n, offset, x) {
    ...
    for (
      i = 1, end1 = m + 1, asc1 = 1 <= end1;
      asc1 ? i <= end1 : i >= end1;
      asc1 ? i++ : i--
    ) {
      var asc1, end1;
      for (
        i = 0, end2 = n - m + 1, asc2 = 0 <= end2;
        asc2 ? i <= end2 : i >= end2;
        asc2 ? i++ : i--
      ) {
        var asc2, end2;
  ...
  eval(x) {
    let result = 0;
    for (
      let start = this.coefs.length - 1, i = start, asc = start <= 0;
      asc ? i <= 0 : i >= 0;
      asc ? i++ : i--
    ) {
  ...
  derivative() {
    ...
    for (
      let i = 1, end = this.degrees(), asc = 1 <= end;
      asc ? i <= end : i >= end;
      asc ? i++ : i--
    ) {
